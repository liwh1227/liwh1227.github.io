...
- `r.Body.Read` 并没有立即返回 `EAGAIN` 错误让我们重试，而是“卡住”直到数据读完。
- `w.Write` 也没有要求我们注册“可写事件”的回调。
...
`w.conn` 结构体中持有一个核心成员 `rwc` (ReadWriteCloser)，它实际上就是 `net.Conn` 接口的实现（通常是 `*net.TCPConn`），也就是`net/http`只是对TCP连接的一层协议包装，核心功能还是在`net`库中实现。

### 1.2 异步非阻塞模式

当我们调用 `net.Conn.Read` 时，调用链会经过 `TCPConn` -> `netFD` -> `poll.FD`。而在`internal/poll/fd_unix.go`中，我们可以发现其异步非阻塞模式的原因。
...
在go的`net`库中针对以往需要（例如cpp编程中）用户手动去处理的内容进行了封装，所以直接调用http包的方法就和阻塞调用一样，其实其底层是非阻塞的，实现原理则是依赖操作系统的 **I/O 多路复用技术**。
...
1. **注册集合**：存储所有通过 `epoll_ctl(ADD/MOD/DEL)` 注册的文件描述符（fd）及其关注的事件类型（如可读/可写/错误/挂起等）。`epoll_ctl` 的职责是对该集合进行增删改，建立或更新“fd → 事件关注”的映射关系。
2. **就绪队列**：当某个 fd 的状态变化并满足关注条件时，内核会将其标记为就绪并加入（或更新）就绪队列，用于汇总当前需要处理的事件。`epoll_wait` 阻塞等待就绪事件：无事件时睡眠直至超时或被唤醒；有事件时将一批就绪事件批量拷贝到用户态并返回。因此用户态的处理成本主要与本次返回的就绪事件数 k相关，而不是与被监控 fd 的总数线性相关。

### 3.2 工作流程

- **创建 epoll 实例**
    
    - 用户态通过 `epoll_create1()` 创建一个 epoll 实例，内核为其初始化并维护对应的数据结构（注册集合/就绪队列）。
    - 该调用返回一个 epoll fd（文件描述符），后续所有操作都围绕这个 epoll fd 进行。
    
- **注册/更新监控对象**
    
    - 用户态调用 `epoll_ctl(epfd, ADD/MOD/DEL, fd, event)` 将目标 fd 及其关注事件注册到 epoll 实例中；
    - 内核据此更新关注集合，建立或更新 “fd → 事件掩码” 的映射关系，对同一 fd 的关注项可通过`MOD`变更，通过`DEL`移除。
    
- **事件就绪与入队**
    
    - 当某个已注册 fd 的状态发生变化并满足关注条件（例如 socket 接收队列变为非空、发送缓冲可写等）时，内核在对应的唤醒路径上将该fd标记为就绪，并将其加入（或更新）到该 epoll 实例的就绪队列。
    
- **等待与批量返回（epoll_wait）**
    
    - 用户态调用 `epoll_wait(epfd, events, maxevents, timeout)` 进入等待：
        
        - 若就绪队列为空，则线程睡眠，直到有事件到来或超时；
...
边缘触发的语义是“**只在状态发生变化的瞬间通知一次**”。以可读事件为例：只有当接收缓冲区从 **“无数据 → 有数据”** 发生跃迁时才触发通知；如果用户态没有把数据读到 `EAGAIN`，后续即使缓冲区里仍然有剩余数据，epoll 也**可能不会再次通知**（除非又有新的数据到达引发新的跃迁）。  
因此 ET 的正确使用方式几乎总是：**fd 必须是 non-blocking，并且读/写需要循环进行，直到返回 `EAGAIN`（或读到 0 表示对端关闭）**。ET 的优势在于：在高活跃连接场景下，它可以减少重复事件回传与无效唤醒，更“事件驱动”；但实现更容易踩坑，常见问题是“没有读到 `EAGAIN` 导致后续不再收到通知”，表现为连接假死或吞吐下降。
