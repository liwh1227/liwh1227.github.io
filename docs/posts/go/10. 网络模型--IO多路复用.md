...
为了支撑海量并发连接，Go并没有采用全局中心化的方式来管理所有的等待 Goroutine，而是建立了一套去中心化的资源绑定体系。在网络模型中，存在严格的 **1:1:1** 映射关系：

>一个网络连接 (`net.Conn`) 对应一个系统文件描述符 (`FD`)，进而对应一个独立的轮询描述符 (`pollDesc`)。

这种架构设计带来了以下关键特性：

1. **分布式状态管理**：当系统中存在 10 万个 TCP 连接时，内存中就会对应存在 10 万个 `pollDesc` 结构体实例。每个 `pollDesc` 仅负责**维护当前这一个连接**的状态和等待队列。
2. **O(1)复杂度的调度**：由于 `pollDesc` 绑定在具体的连接对象上，当 `epoll_wait` 返回就绪事件时，内核直接通过 `epoll_data` 指针返回该 FD 对应的 `pollDesc` 内存地址。Runtime 可以直接访问该结构体的 `rg` 字段找到目标 G，整个过程不随连接数的增加而变慢。 
3. **竞态隔离**：不同连接的 I/O 操作完全隔离。Connection A 的读写操作只会访问 Connection A 独享的 `pollDesc`，绝不会干扰 Connection B 的状态。这意味着锁的粒度被分散到了每个连接级别，极大地降低了锁竞争。

映射关系示意图如下所示：

![](../../assets/10-网络模型-映射关系.png)
*图3: netpoller映射关系*
