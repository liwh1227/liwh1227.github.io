...
1. **消除全局锁瓶颈：** 每个 P 维护一个**本地运行队列**。M 优先消费本地队列，无需加锁，大幅提升了并行度。
...
* **溢出兜底：** 全局队列还充当了溢出缓冲区，当 P 本地队列满（256 slots）[^5]时，部分 G 会被批量转移至全局队列。
...
-  **异步抢占 (Go 1.14+)：** 针对没有函数调用的密集计算循环，Sysmon 发送 `SIGURG` 信号，通过信号处理函数强行中断并保存上下文。
