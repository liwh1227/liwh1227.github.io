...
这种设计可以提升垃圾回收的效率，减少垃圾回收的成本。并且通过partial和full列表对内存进行管理，进一步提升垃圾回收的效率，当mcache进行内存申请时，优先从partial列表获取，然后才会去获取full列表中的span。` partial/full [2]spanSet`是一个大小为2的数组，并不是1，在源码中，有这么一段逻辑：
```go
...
1. span 会被移回 `mcentral`。
2. 如果 span 还有空位，放入 `partial` 集合。
3. 如果 span 满了，放入 `full` 集合。
...
4. **返回给 cacheSpan**：作为进货结果交给触发 refill 的 `mcache`，从而恢复小对象分配快路径。

## 5. 总结

`mcentral` 是 Go 内存分配器中连接mheap和mcache的关键环节。
