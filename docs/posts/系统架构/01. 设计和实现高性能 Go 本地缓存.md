...
> 本着干中学的理念，在梳理完 go map 的相关知识后，针对其存在的痛点，并根据某些应用场景中可以优化的点来设计和实现一个高性能的 cache 库——[tinycache](https://github.com/liwh1227/tinycache)。
...
- **内存只增不减**：正如[15. map](../go/15.%20map.md) 中分析，map 只扩不缩。在缓存场景中，流量高峰期 map 扩容后，低峰期即使大量条目被淘汰，底层 bucket 数组的内存也不会释放。
...
TinyCache 是对 Go 语言海量数据缓存场景下 GC 压力与锁竞争痛点的一次深度探索与重构。它继承了 BigCache 的零 GC 核心理念，但在数据模型与内存调度上进行了泛型化与结构化升级。

通过将 `[]byte` 字节流队列替换为 `[]slot[T]` 随机访问数组，TinyCache 彻底消除了序列化与反序列化过程中的 CPU 及临时堆分配开销。同时，通过引入 FreeList 栈结构，它打破了传统 FIFO 队列依赖自然淘汰的局限，实现了底层内存空间的 $O(1)$ 即时回收与复用，显著提升了 CPU 缓存的局部性命中率。

尽管这种极致的优化引入了“泛型 T 必须严格为纯值类型”的刚性约束，并在极端哈希碰撞场景下做出了原址覆盖的妥协，但这种系统设计上的取舍，为已知结构体缓存（如用户画像、行情快照等场景）带来了接近硬件极限的吞吐量与极低的延迟抖动。在高性能架构工程中，往往没有绝对完美的银弹，TinyCache 证明了通过精准剪裁冗余功能，可以为特定领域带来数量级上的性能飞跃。